...
<div class="container">
	...
	<?php
		...
		$numero = $_POST["numero"];
		$radionom = $_POST["radionom"];
		$radioprenom = $_POST["radioprenom"];
		$radionumero = $_POST["radionumero"];
		...
		else
		{
			...
			if (!empty($civilite))
			{
				$criteres1 .= " AND annuaire_exploit_abonne.id_Pcivilite='".$civilite."'";
			}
			...
			if (!empty($service))
			{
				...
				$criteres1 .= " AND annuaire_exploit_service.id_Eabonne=annuaire_exploit_abonne.id_Eabonne AND annuaire_exploit_service.id_Pservice=annuaire_param_service.id_Pservice AND annuaire_param_service.id_Pservice='".$service."'";
			}
			
			// SI LE CHAMP 'FONCTION' EST REMPLI
			if (!empty($fonction))
			{
				$criteres1 .= " AND annuaire_exploit_abonne.id_Pfonction='".$fonction."'";
			}
			
			// SI LE CHAMP 'NUMERO' EST REMPLI
			if (!empty($numero))
			{
				$sqltable .= ",annuaire_exploit_numero";
				
				// SI LA RADIOBOX 'COMMENCE PAR' EST COCHEE
				if($radionumero == "radionumero1")
				{
					$criteres1 .= " AND annuaire_exploit_abonne.id_Eabonne=annuaire_exploit_numero.id_Eabonne AND lib_num LIKE '".$numero."%'";
				}
		...
		// REQUETE CRITERES 1
		$sqlcriteres1 = "SELECT * FROM annuaire_exploit_abonne".$sqltable."
								WHERE actif_ab=1
								";
