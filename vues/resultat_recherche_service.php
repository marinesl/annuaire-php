...
<head>
	...
	<title>Annuaire PHP - Recherche par service</title>
	...
	<div class="container">
		...
		<?php
			...
			if ((!empty($_POST['inputcritere'])) OR (isset($_POST['inputcritere']))) {
				...
				// CRITERES 1
				$criteres1.= " AND annuaire_php_exploit_service.id_Eabonne = annuaire_php_exploit_abonne.id_Eabonne 
								AND annuaire_php_exploit_service.id_Pservice = annuaire_php_param_service.id_Pservice
								AND annuaire_php_param_service.lib_ser LIKE '%".$service."%'";
				$sqltable .= ", annuaire_php_param_service, annuaire_php_exploit_service";
			}
			
			// REQUETE CRITERES 1 
			$sqlcriteres1 = "SELECT * 
							FROM annuaire_php_exploit_abonne,annuaire_php_param_service,annuaire_php_exploit_service
							WHERE actif_ab = 1 ";
			...
			$resultresultat = ($queryresultat->rowCount() === 0) ? 0 : $queryresultat->fetchAll();
		...
		<div class="row">	
			...
			<?php
				$sqlpresentation = "SELECT * 
									FROM annuaire_php_exploit_abonne, annuaire_php_param_pole, annuaire_php_param_ug, annuaire_php_param_localisation, annuaire_php_param_batiment, annuaire_php_param_etage, annuaire_php_param_porte".$sqltable."
									WHERE annuaire_php_param_pole.id_Ppole = annuaire_php_param_service.id_Ppole
									AND annuaire_php_param_ug.id_Pservice = annuaire_php_param_service.id_Pservice
									AND annuaire_php_param_localisation.id_Plocalisation = annuaire_php_param_service.id_Plocalisation
									AND annuaire_php_param_localisation.id_Pbatiment = annuaire_php_param_batiment.id_Pbatiment
									AND annuaire_php_param_localisation.id_Petage = annuaire_php_param_etage.id_Petage
									AND annuaire_php_param_localisation.id_Pporte = annuaire_php_param_porte.id_Pporte";
				$sqlpresentation .= $criteres1;

				$querypresentation = $connectBdd->prepare($sqlpresentation);
				$querypresentation->execute();
				$resultpresentation = ($querypresentation->rowCount() === 0) ? 0 : $querypresentation->fetchAll();

				if ($resultpresentation !== 0) {
					for ($i = 0 ; $i < count($resultpresentation) ; $i++) {
			?>
					<div class="panel panel-primary">	
						<div class="panel-heading">					
							<h4 class="panel-title">
								<?php 
									...
									echo $resultpresentation[$i]['lib_ser']." (".$resultpresentation[$i]['num_ser'].")"; 
								?> 
							</h4>
						</div>
						<div class="panel-body">
								...
								<div class="col-lg-5">
									<ul class="list-unstyled">
										<?php 
											...
											echo "<li>Responsable : <b>".$resultpresentation[$i]['responsable_ser']."</b></li>";
											
											// AFFICHAGE DES POLES
											echo "<li>Pôle : ".$resultpresentation[$i]['lib_pole']." (".$resultpresentation[$i]['num_pole'].")</li>";
											
											// AFFICHAGE DES UG
											echo "<li>UG : ".$resultpresentation[$i]['lib_ug']." (".$resultpresentation[$i]['num_ug'].")</li>";
										?>								
									</ul>
								</div>
								...
								<div class="col-lg-4">							
									<ul class="list-unstyled">							
										<?php
											...
											$sqlnumero1 = "SELECT * 
															FROM annuaire_php_exploit_numero,annuaire_php_param_communication
															WHERE annuaire_php_exploit_numero.id_Pservice='".$resultpresentation[$i]['id_Pservice']."'
											...
											$resultnumero1 = ($querynumero1->rowCount() === 0) ? 0 : $querynumero1->fetchAll();

											if ($resultnumero1 !== 0) {
												for ($j = 0 ; $j < count($resultnumero1) ; $j++) {
													echo("<li>Accueil : ".$resultnumero1[$j]['lib_num']."</li>\n");	
												}
											}
											...
											$resultnumero2 = ($querynumero2->rowCount() === 0) ? 0 : $querynumero2->fetchAll();

											if ($resultnumero2 !== 0) {
												for ($j = 0 ; $j < count($resultnumero2) ; $j++) {
													echo("<li>Poste de soins : ".$resultnumero2[$j]['lib_num']."</li>\n");	
												}
											}
										?>
									</ul>							
								...
								<div class="col-lg-3">
									<ul class="list-unstyled">
										<?php 
											...
											echo "<li>Batiment : ".$resultpresentation[$i]['lib_bat']."</li>";
											echo "<li>Porte : ".$resultpresentation[$i]['lib_porte']."</li>";
											echo "<li>Etage : ".$resultpresentation[$i]['lib_eta']."</li>";
									?>
									</ul>
								</div>	
			...
			<?php
				if ($resultresultat !== 0) {
					for ($i = 0 ; $i < count($resultresultat) ; $i++) {
			?>		
					<div class="panel panel-primary">	
						<div class="panel-heading">					
							<h4 class="panel-title">
							<?php 
								...
								echo $resultresultat[$i]['lib_civ']." ".$resultresultat[$i]['nom_ab']." ".$resultresultat[$i]['prenom_personne'];
							?> </h4>
						</div>
						<div class="panel-body">
							<div class="row">
									...
									<ul class="list-unstyled">
										<?php 
											...
											$sqlservice2 = "SELECT distinct(lib_ser),num_ser
															...
															WHERE annuaire_php_exploit_abonne.id_Eabonne='".$resultresultat[$i]['id_Eabonne']."'
															AND annuaire_php_exploit_service.id_Eabonne='".$resultresultat[$i]['id_Eabonne']."'
															AND annuaire_php_exploit_service.id_Pservice=annuaire_php_param_service.id_Pservice
															ORDER BY lib_ser";
											...
											echo "<li>".$resultresultat[$i]['lib_fonc']."</li>";
											
											// AFFICHAGE DE LA LOCALISATION 
											echo "<li>Bâtiment : ".$resultresultat[$i]['lib_bat']."</li>";
											echo "<li>Etage : ".$resultresultat[$i]['lib_eta']."</li>";
											echo "<li>Porte : ".$resultresultat[$i]['lib_porte']."</li>";
										?>								
									</ul>
								</div>
								...
								<div class="col-lg-4">							
									<ul class="list-unstyled">							
										<?php
											// REQUETE SQL + AFFICHAGE DES NUMEROS 
											$sqlnumero = "SELECT * 
															FROM annuaire_php_exploit_abonne,annuaire_php_exploit_numero
															WHERE annuaire_php_exploit_abonne.id_Eabonne='".$resultresultat[$i]['id_Eabonne']."'
															AND annuaire_php_exploit_numero.id_Eabonne='".$resultresultat[$i]['id_Eabonne']."'
															ORDER BY ordre_num";
								...
								<div class="pull-right">
									<ul class="list-unstyled">
										...
										<?php 
											...
											//echo $resultresultat[$i]['photo_personne'];
											// if(!empty($resultresultat[$i]['photo_personne']))
											// {
												// echo $resultresultat[$i]['photo_personne'];
											// }
											// else
